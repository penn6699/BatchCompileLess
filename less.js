            ...
            for (var i = 0; i < childArray.length; i++) {
                ...
                if (stats.isDirectory()) {
                    ...
                    if (path.extname(currentFilePath) === ".less") {
                        var newFilePath = path.resolve(targetPath, path.basename(currentFilePath, '.less'));
                        if (!fs.existsSync(targetPath)) {
                            fs.mkdirSync(targetPath);
                        }
                        console.log(newFilePath+ ".css");
                        exec("lessc " + currentFilePath + " > " + newFilePath+ ".css");
						console.log(newFilePath+".min.css");
						exec("lessc -x " + currentFilePath + " > " + newFilePath+ ".min.css");
                    }
                }
            }
        }
